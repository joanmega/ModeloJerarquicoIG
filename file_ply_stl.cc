...
void leer_cabecera( ifstream & src, unsigned & num_vertices, unsigned & num_caras ) ;
...
)
{
   ...
   leer_cabecera( src, num_vertices, num_caras ) ;
...
void leer_cabecera( ifstream &src, unsigned & num_vertices, unsigned & num_caras )
{
   ...
   if ( nv <= 0 /*|| nc <= 0*/ )
      error("no se ha encontrado el número de vértices o caras, o bien alguno de los dos es 0 o negativo");
